...
export default function BlindpotDrawHistory() {
  ...
  const handleClaim = async (drawId: string) => {
    ...
    setErrorMsg(null);
    setStatusMsg(`Submitting blinded claim for Draw #${drawId}...`);

    try {
      await claim(VAULT_ADDRESS, BigInt(drawId));
      setStatusMsg(`Claim transaction confirmed for Draw #${drawId}! If you were the winner, your winnings have been transferred confidentially.`);
    } catch (e: any) {
      console.error(e);
      setErrorMsg(e?.message || `Claim failed for Draw #${drawId}. Make sure the draw has finished and you haven't already claimed.`);
      setStatusMsg(null);
    } finally {
  ...
  return (
    <>
      ...
      <main className="flex-grow pt-24 md:pt-28 pb-20 px-margin-mobile md:px-margin-desktop max-w-[1100px] mx-auto w-full">
        <div className="mb-8 border-2 border-primary bg-surface p-gutter hard-shadow-primary">
          ...
          <div className="font-label-mono text-xs text-on-surface-variant uppercase bg-surface-container-low border border-primary border-dashed p-3">
            <span className="text-primary font-bold">PUBLIC DISCLOSURE:</span> DRAW ID · BLOCK TIMESTAMP · VERIFIABLE PROOF.<br />
            <span className="text-error font-bold">PRIVATE &amp; BLINDED:</span> WINNING AMOUNT · LOSING BALANCES · CLAIMER IDENTITY.
          </div>
        </div>
                  ...
                  return (
                    <tr key={drawNum} className="hover:bg-surface-container-low transition-colors">
                      ...
                      <td className="p-3.5 border-r border-primary/20 font-mono text-xs">
                        FHE.randEuint (deposit-weighted)
                      </td>
                      ...
                      <td className="p-3.5 border-r border-primary/20 text-center">
                        <button
                          ...
                          disabled={isClaimingThis || !isConnected}
                        >
